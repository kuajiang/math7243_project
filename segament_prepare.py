...
def table_label(table):
    hemorrhage_type = table.loc[0,'hemorrhage_type']
    for _ in range(len(table)):
        pic = np.full((512,512), False,dtype=bool)
        ...
        for i in range(len(pics)):
            if type(correct[i])==type(np.nan):
                ...
                if type(string)==type(np.nan):
                    img_pil = Image.fromarray(pic.astype(np.uint8) * 255)
                    img_pil.save('./label/'+hemorrhage_type+'/label_'+table['Origin'][_][:-4]+'.png')
                    return None
...
table4 = pd.read_csv('./labels/Results_Subarachnoid_Hemorrhage_Detection_2020-11-16_21.36.18.668.csv')
table4['Correct Label'] = table4['Correct Label'].replace('[[]]',np.nan)
table4['Majority Label'] = table4['Majority Label'].replace('[]',np.nan)

table5 = pd.read_csv('./labels/Results_Subdural_Hemorrhage_Detection_2020-11-16_21.35.48.040.csv')
